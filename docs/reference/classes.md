...
By default, the class generated by the Kotlin compiler will have one constructor which takes values for all properties
of this class as constructor arguments. In order to use a class with libraries, that require the objects to have a
parameterless constructor (for example Jackson or JPA), it you need to specify default values for each property:

``` kotlin
class Customer(val customerName: String = "")
```

The compiler will then generate an additional parameterless constructor for the class. This parameterless constructor
