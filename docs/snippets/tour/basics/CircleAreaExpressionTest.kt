...
// whether circleArea returns a Double or an Int. The helpers are an
// independent copy of the ones in CircleAreaTest.kt - the exercise files are
// never compiled together, so the private top-level names don't clash.
...
class CircleAreaExpressionTest {
    // Known limitation: whether the function is written in single-expression
    // form cannot be observed at runtime - a block-body solution from the
    // previous exercise passes too.
