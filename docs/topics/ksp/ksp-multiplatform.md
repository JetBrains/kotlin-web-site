[//]: # (title: KSP for Multiplatform)
[//]: # (description: ...)

This document explains how to use KSP in a Kotlin Multiplatform project. To get started with Kotlin Multiplatform, see 
the [Kotlin Multiplatform overview](https://kotlinlang.org/docs/multiplatform/kmp-overview.html).
...
or other build flavors). Symbol processing may also happen multiple times. Whenever there is a Kotlin compilation task 
whose target is marked for KSP processing, a corresponding symbol processing task is created.
...
Therefore, in this example, there are at least 12 compilation tasks and 10 symbol processing tasks created: one for 
