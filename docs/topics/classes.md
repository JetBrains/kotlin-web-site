...
* **Class body** (optional), surrounded by curly braces `{}`, and including class members:
...
Here's an example of a class declaration with a property:

```kotlin
// Class header with a primary constructor that initializes the 'name' property
class Person(val name: String) {
    // Class body with the `age` property 
    var age: Int = 0
}
```
...
[object creation](#creating-objects-of-classes).
...
Declare the primary constructor in the class header after the class name:
...
This primary constructor ensures that when [creating an object](#creating-objects-of-classes) of `Person`, a 
value for `name` must be provided, defining an initial required input.
...
You can define simple properties directly in the primary constructor. This approach makes it easy to create
classes that need properties initialized directly during object creation.

To declare a read-only property, use the `val` keyword in the parentheses before the argument name. 
...
A class can define member [functions](functions.md) that, in turn, access class properties.
To make a primary constructor parameter accessible in member functions, declare it as a property using `val` or `var`:
...
initialized in the class body using the constructor parameters.
...
You can add as many initializer blocks as you need, and they will be executed in the order in which they appear in the class body, 
interleaved with property initializers (`init{}`).
...
Secondary constructors are useful when interacting with Java code that requires traditional constructors or if
...
class Person(val name: String, var age: Int) {
    ...
    constructor(name: String) : this(name, 8) {
        println("$name created with default age: $age")
        // Bob created with default age: 8
    }
}

fun main() {
    // Creates an object using the secondary constructor, which sets 'age' to 8
    Person("Bob")
}
```
{kotlin-runnable="true"}
...
> For further information about delegation via superclass constructor, see [Calling the superclass implementation](inheritance.md#calling-the-superclass-implementation).
>
{style="note"}
...
class Person {
    ...
    init {
        ...
        println("Initializer block executed")
...
If there's a class that is a [non-abstract class](#abstract-classes) and does not declare any constructors (primary or secondary), it will have a generated primary constructor
with no arguments:
...
fun main() {
    // Creates an object of the 'Person' class using the automatically generated primary constructor
    val person = Person()
}
```
...
> On the JVM, if all primary constructor parameters have default values, the compiler generates 
> an additional parameterless constructor that uses those default values. 
> 
...
>
> In the following example, Kotlin automatically generates a parameterless constructor `Person()` that uses the default value `""`:
> 
> ```kotlin
...
When the program runs, execution starts from the `main()` function, so this is where you instantiate objects to interact with them.
When creating objects of a class, you're invoking constructors (for example, `Person()` or `Person("Joe")`) directly inside
the `main()` function.
...
```kotlin
// Abstract class with a primary constructor that defines 'name'
abstract class Person(val name: String) {
  // Non-abstract open member that allows overriding
  open fun greet() {
    println("Hello, my name is $name.")
  }
...
// Subclass that overrides the 'greet()' function with an abstract one
abstract class Student(name: String, val school: String) : Person(name) {
  // Overrides the non-abstract 'greet()' function with an abstract one
  abstract override fun greet()

  // Overrides the abstract 'introduce()' function with an abstract one
  override fun introduce() {
    println("I am $name, and I study at $school.")
  }
}
