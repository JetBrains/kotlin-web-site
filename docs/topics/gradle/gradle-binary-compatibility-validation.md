...
To update the reference ABI dump that Gradle uses to check your latest changes against, run the `updateLegacyAbi` task in IntelliJ 
...
You can define filters to control which declarations (classes, properties, functions) the ABI dump includes. Use the `filters {}`
block to add exclusion and inclusion rules with `excluded {}` and `included {}` blocks respectively.

Gradle includes a declaration in the ABI dump only if it passes **all** exclusion and inclusion rules.
