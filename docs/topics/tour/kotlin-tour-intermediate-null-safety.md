...
to infer the type without explicit declaration. When you tell Kotlin to treat a variable or object as if it belongs to a
specific type, this process is called **casting**. When a type is automatically cast, like when it's inferred, it's called
**smart casting**.

Before we explore how casting works, how can you check if an object has a certain type? For this, you can use the
...
In the reduced version, the example uses the [`.sumOf()`](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin.collections/sum-of.html) extension function and provides a lambda expression that:
...
There are also functions that use a lambda expression to transform a collection and return `null` values if they can't 
...
fun main() {
    ...
    // Calculate the total price using reduceOrNull
...
* There is a `User` data class that has properties for the `id`, `name` and a list of friends for the user.
...
1. The `validUser` variable uses the `as?` operator to check if `user` is a `User` object. If it isn't, return an empty list.
...
* The amount in the `requested` variable is higher than the amount in the `available` variable.
