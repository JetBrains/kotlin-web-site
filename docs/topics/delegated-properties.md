...
Under the hood, the Kotlin compiler may generate auxiliary properties for some kinds of delegated properties and then delegates to them.
...
In the following example, the compiler generates the hidden property `prop$delegate`, and the accessors
delegate to this additional property:
