...
| `jvmTarget`       | `kotlin.compiler.jvmTarget`       | Target JVM version of the generated bytecode. Controls only the bytecode version of the output and does not restrict which JDK APIs your code can use.                                                  | "1.8", "9", "10", ..., "26"                                              | "%defaultJvmTargetVersion%" |
| `jdkRelease`      | `kotlin.compiler.jdkRelease`      | Target JVM version. Controls the bytecode version and limits the available API to the specified JDK version, preventing accidental use of newer APIs. Equivalent to Java's `--release` compiler option. | "1.8", "9", "10", ..., "26"                                              |                             |
...
| `-Xadd-modules`   |                                   | (Experimental) Resolve the specified root modules in addition to the initial modules. Set the `ALL-MODULE-PATH` value to resolve to all modules on the module path.                                     | Comma-separated module names or `ALL-MODULE-PATH` passed through `<arg>` |                             |
