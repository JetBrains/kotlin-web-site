...
| `jdkRelease`      | `kotlin.compiler.jdkRelease`      | Target JVM version. Controls both the bytecode version and limits the available API to the specified JDK version, preventing accidental use of newer APIs. Equivalent to Java's `--release` compiler option. | "1.8", "9", "10", ..., "26"                             |                             |
