...
> Java introduced [a similar concept](https://docs.oracle.com/en/java/javase/15/language/sealed-classes-and-interfaces.html#GUID-0C709461-CC33-419A-82BF-61461336E65F) 
> in Java 15, where sealed classes use the `sealed` keyword along with the `permits` clause to define restricted hierarchies.
>
{type="tip"}
...
but for its subclasses. Let's look at an example where we have a sealed class called `Error` with several subclasses, 
...
In such cases, you don't need to add an `else` clause. Here's an example demonstrating this:
...
Let's explore some practical scenarios where the application of sealed classes or interfaces can be particularly helpful.
