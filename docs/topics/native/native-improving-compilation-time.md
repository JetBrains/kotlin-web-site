...
* **Use the compiler caches**. Starting from 1.5.0-M1, `linuxX64` and `iosArm64` targets have experimental opt-in support
  for compiler caches that improve compilation time for debug builds (for `linuxX64`, this feature is only available on Linux hosts).
  You can use them by adding `kotlin.native.cacheKind.linuxX64=static` or `kotlin.native.cacheKind.iosArm64=static` to `gradle.properties`.

  `iosX64` and `macosX64` targets already have the compiler caches enabled by default.

* **Enable previously disabled features of Kotlin/Native**. If you have the `kotlin.native.disableCompilerDaemon=true` or
