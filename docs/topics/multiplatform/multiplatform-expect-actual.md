...
and `iosSimulatorArm64Main`platform source sets. Only `iosMain` typically contains the expect declarations while actual declarations are provided in the platform-specific source sets. The Kotlin compiler will then use these actual declarations to produce the resulting code for the
