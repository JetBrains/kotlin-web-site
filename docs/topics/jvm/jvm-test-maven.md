...
values in your assertions, giving you the full diagrams in the console output.

1. To enable Power-assert, add the following configuration to your project's `pom.xml`:
    ...
    ```
    {initial-collapse-state="collapsed" collapsible="true" ignore-vars="false" collapsed-title="kotlin-maven-power-assert configuration"}
...
2. Create a test file, `UserProfileBackendTest.kt`, in the `src/test/kotlin` directory:
...
The console displays the full Power-assert diagram, showing the difference between the expected and actual values at
each position in the expression:
...
* Learn more about the [Power-assert compiler plugin](power-assert.md).
