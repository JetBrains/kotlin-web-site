...
library is added to the rest. The Kotlin Gradle plugin will select the appropriate JVM standard library depending on the `jvmTarget` value specified 
...
If you do not need to use the standard library in your project, you can add the opt-out flag to the `gradle.properties`:
...
### Compiler option attributes

The complete list of options for the Gradle tasks is the following:

| Name | Description | Possible values |Default value | Platforms |
|------|-------------|-----------------|--------------| ---------|
| `allWarningsAsErrors` | Report an error if there are any warnings |  | false | JVM, JS |
| `suppressWarnings` | Generate no warnings |  | false |  JVM, JS |
| `verbose` | Enable verbose logging output |  | false | JVM, JS |
| `freeCompilerArgs` | A list of additional compiler arguments |  | [] | JVM, JS |
| `apiVersion` | Allow using declarations only from the specified version of bundled libraries | "1.2" (DEPRECATED), "1.3", "1.4", "1.5" (EXPERIMENTAL) |  |  JVM, JS |
| `languageVersion` | Provide source compatibility with the specified version of Kotlin | "1.2" (DEPRECATED), "1.3", "1.4", "1.5" (EXPERIMENTAL) |  | JVM, JS |
| `javaParameters` | Generate metadata for Java 1.8 reflection on method parameters |  | false | JVM |
| `jdkHome` | Include a custom JDK from the specified location into the classpath instead of the default JAVA_HOME |  |  | JVM |
| `jvmTarget` | Target version of the generated JVM bytecode | "1.6", "1.8", "9", "10", "11", "12", "13", "14", "15" | "1.6" | JVM |
| `noJdk` | Don't automatically include the Java runtime into the classpath |  | false | JVM |
| `noReflect` | Deprecated. Don't automatically include Kotlin reflection into the classpath |  | true | JVM |
| `noStdlib` | Deprecated. Don't automatically include the standard library into the classpath. Learn more about the [dependency on the standard library](#dependency-on-the-standard-library). |  | true | JVM |
| `useIR` | Use the JVM IR backend |  | false | JVM |
| `friendModulesDisabled` | Disable internal declaration export |  | false | JS |
| `main` | Define whether the `main` function should be called upon execution | "call", "noCall" | "call" | JS |
| `metaInfo` | Generate .meta.js and .kjsm files with metadata. Use to create a library |  | true | JS |
| `moduleKind` | The kind of JS module generated by the compiler | "umd", "commonjs", "amd", "plain"  | "umd" | JS |
| `outputFile` | Destination *.js file for the compilation result |  | "\<buildDir>/js/packages/\<project.name>/kotlin/\<project.name>.js" | JS |
| `sourceMap` | Generate source map |  | true | JS |
| `sourceMapEmbedSources` | Embed source files into source map | "never", "always", "inlining" |  | JS |
| `sourceMapPrefix` | Add the specified prefix to paths in the source map |  |  | JS |
| `target` | Generate JS files for specific ECMA version | "v5" | "v5" | JS |
| `typedArrays` | Translate primitive arrays to JS typed arrays |  | true | JS |
