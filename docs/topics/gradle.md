...
Old behaviour, when the `jdkHome` option is not set, is still working and still, by default, using the current user's JDK version.
...
Or you can specify the path to your local JDK and its version.
