...
|-----------------------------------------------------------|----------------------------------------------------------|
| [Main page](https://kotlinlang.org/)                      | [pages/index.tsx](pages/index.tsx)                       |
...
- Your local environment is connected to the WebTeam registry. Learn more about it [in the section below](#local-development).
- Your Node.js version matches `.nvmrc` (using [`nvm`](https://github.com/nvm-sh/nvm) or alternatives like [`n`](https://github.com/tj/n), [`asdf`](https://github.com/asdf-vm/asdf), [`mise`](https://github.com/jdx/mise)).

To generate a PDF version of the Kotlin documentation:

1. Download and extract the `pages.zip` artifacts from the [Build site pages CI build](https://buildserver.labs.intellij.net/buildConfiguration/Kotlin_KotlinSites_KotlinlangTeamcityDsl_BuildSitePages) into the `dist` directory:
   ```bash
   mkdir -p dist
   unzip pages.zip -d dist
   ```
   This will provide the required `dist/docs` directory.
2. Run:
   ```bash
...
The PDF will be created at `assets/kotlin-reference.pdf`. Generation may take several minutes. (Team members can also run or download artifacts directly from the [PDF Generator CI build](https://buildserver.labs.intellij.net/buildConfiguration/Kotlin_KotlinSites_KotlinlangTeamcityDsl_PdfGenerator)).
