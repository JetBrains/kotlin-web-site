...
def process_nav_includes(data):
    ...
    if isinstance(data, dict):
        ...
        if external_key in data:
            ...
            external_base: str = external_spec['base']
            external_path: str = external_spec['path']
            external_nav: str = external_spec['nav']
            external_repo: str = external_spec['repo']
            external_branch: str = external_spec['branch']
            del data[external_key]

            assert external_repo.startswith("https://github.com/JetBrains")
            ...
            def create_function_to_replace_urls(url, md):
                pattern = "\\]\\("        "(" + re.escape(md) + ")"           "(#[^\\)]+)?"     "\\)"
                return lambda text: re.compile(pattern).sub("](" + url + "\\2)", text)

            for item in external_yml:
                url_mappers.append(create_function_to_replace_urls(item['url'], item['md']))
            ...
            for item in external_yml:
                ...
                github_edit_url = external_repo.rstrip('/') + "/edit/" + external_branch + "/" + md.lstrip("/")
                ...
                with open(target_item, 'w') as file:
