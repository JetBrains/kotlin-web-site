...
export const QuoteCard: FC<QuoteCardProps> = ({
    ...
    return (
        <div className={cn(styles.card, className, textCn('rs-text-1'))} data-testid={testId}>
            ...
            <p className={cn(styles.quote, textCn('rs-text-1', {hardness: 'hard'}))}>{quoted ? <>&ldquo;{quote}&rdquo;</> : quote}</p>
            {author && (
                <div className={styles.attribution}>
                    <p className={cn(styles.author, textCn('rs-h3'))}>{author}</p>
                    {role && <p className={cn(styles.role, textCn('rs-text-2', {hardness: "hard"}))}>{role}</p>}
                </div>
            )}
            {linkText && (
                <a
