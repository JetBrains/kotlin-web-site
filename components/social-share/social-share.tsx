...
export interface SocialShareProps {
    ...
    title?: string;                             // Title for social media share text
    theme?: SocialShareTheme;                   // Visual theme
    size?: SocialShareSize;                     // Icon size
    className?: string;
    networks?: SocialNetwork[];                // Which network to show (defaults to all)
    label?: string;                             // Optional label like "Share:"
}
...
export const SocialShare: FC<SocialShareProps> = (
    ...
    return (
        <div className={cn(styles.socialShare, styles[`theme_${theme}`], styles[`size_${size}`], className)}>
            {label && <span className={cn(styles.label, textCn('rs-text-2', { hardness: 'average' }))}>{label}</span>}
