...
1. In the `plugins` block within `build.gradle` (`build.gradle.kts`), change the `KOTLIN-EAP-VERSION` to the actual EAP version, such as `{{ site.data.releases.latest.eap-version }}`. Available EAP versions are listed [here](index.html#build-details).<br>
...
1. In `build.gradle` (`build.gradle.kts`), change the `KOTLIN-EAP-VERSION` in the `buildscript` block to the actual EAP version, such as `{{ site.data.releases.latest.eap-version }}`.  
...
For the **kotlinx.coroutines** library, add the version number – `{{ site.data.releases.latest.coroutines.eap-version }}` – that is compatible with `{{ site.data.releases.latest.eap-version }}`. 
...
```groovy
dependencies {
   implementation "org.jetbrains.kotlinx:kotlinx-coroutines-core:{{ site.data.releases.latest.coroutines.eap-version }}"
}
```
...
In the sample Maven project definition, replace `KOTLIN-EAP-VERSION` with the actual version, such as `{{ site.data.releases.latest.eap-version }}`. 
Available EAP versions are listed [here](index.html#build-details).
