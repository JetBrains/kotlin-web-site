...
The `provideDelegate` operator method can be used to extend the logic of creating the object to which the property implementation is delegated.
...
One of the possible uses is to check property consistency when the property is created, not only in its getter or setter.
...
class ResourceLoader<T>(id: ResourceID<T>) {
    operator fun provideDelegate(thisRef: MyUI,
                                 property: KProperty<*>): ReadOnlyProperty<MyUI, T> {
...
we'd have to pass the property name explicitly, which isn't very convenient:
...
fun <T> MyUI.bindResource(id: ResourceID<T>,
                          propertyName: String): ReadOnlyProperty<MyUI, T> {
   checkProperty(this, propertyName)
   // create delegate
}
```

In the generated code, the `provideDelegate` method is called to initialize the auxiliary `c$delegate` property.
Compare the generated code for the property declaration `val prop: Type by MyDelegate()` with the generated code above (when the `provideDelegate` method is not present):
...
The `provideDelegate` method affects only the creation of the auxiliary property and doesn't affect the code generated for getter or setter.
