...
The operator method `provideDelegate` can be used to check property consistency when property is created, not only in its getter or setter.

If the `provideDelegate` method is present, the Kotlin compiler will use it to create the auxiliary `c$delegate` property.
...
Without this ability to intercept the binding between the property and its delegate, to achieve the same functionality we'd have to pass the property name explicitly, which isn't very convenient:
