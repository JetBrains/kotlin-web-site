...
When the processing of an `Iterable` includes multiple steps, they are executed eagerly: each intermediate step completes and returns its result – an intermediate collection.
The following step executes on this collection.

In turn, multi-step processing of sequences is executed lazily when possible: actual computing happens only on calling a terminal operation that returns the result of the whole processing chain, such as `toList()` or `sum()`.
...
The sequence generation stops when the provided function returns `null`. So, the first sequence in the example below is infinite.
...
Assume that you have a list of words. The code below filters the words longer than three characters and prints the lengths of first five such words.
